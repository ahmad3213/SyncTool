...
struct PhysicalValue {
  ...
  PhysicalValue(ValueType value, ValueType error) : value(value), error(error) {}
  PhysicalValue()
    : value(std::numeric_limits<ValueType>::quiet_NaN()), error(std::numeric_limits<ValueType>::quiet_NaN()) {}
  ValueType value, error;

  template<typename char_type>
  std::basic_string<char_type> ToString(bool print_stat_error) const
  {
    ...
    const int precision = error != 0.
      ? static_cast<int>(std::floor(std::log10(error)) - number_of_significant_digits_in_error + 1) : -15;
    const ValueType ten_pow_p = std::pow(10.0, precision);
    const ValueType stat_error_rounded = std::ceil(error / ten_pow_p) * ten_pow_p;
...
inline std::ostream& operator<<(std::ostream& s, const PhysicalValue& v)
{
    s << v.ToString<char>(true);
    return s;
}

inline std::wostream& operator<<(std::wostream& s, const PhysicalValue& v)
{
    s << v.ToString<wchar_t>(true);
    return s;
}

std::istream& operator>>(std::istream& s, PhysicalValue& r)
{
    PhysicalValue::ValueType value, error;
    s >> value >> error;
    if(s.fail())
        throw exception("Invalid physical value.");
    r = PhysicalValue(value, error);
    return s;
}
