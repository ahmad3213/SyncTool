...
inline std::ostream& operator <<(std::ostream& s, const Condition& cond)
{
    if(cond.always_true)
        s << 1;
    else {
...
inline std::istream& operator >>(std::istream& s, Condition& cond)
{
...
inline std::ostream& operator <<(std::ostream& s, const SyncPlotEntry& entry)
{
...
inline std::istream& operator >>(std::istream& s, SyncPlotEntry& entry)
{
    std::string line;
    std::getline(s, line);
